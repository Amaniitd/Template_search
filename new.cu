...
__global__ void filter(int *data_image, int *range, float query_grey, int subrow, int col, int query_row, int query_col, float TH2, char *filtered, int start_row, int real_row)
{ // filer the candidates for calculting the RMSD
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int x = blockIdx.x*blockDim.x + threadIdx.x;
    if (x >= subrow || y >= col - query_col || x+start_row>=real_row - query_row){
        return;
    }
    ...
    { // grey avg of image
        for (int X = x + range[6 + theta]; X < x + range[9 + theta]; X++){
            for (int color = 0; color < 3; color++){
                if (X >= 0 && Y >= 0 && X < subrow && Y < col){
                    grey_sum += data_image[(subrow - 1 - X) * (col)*3 + Y * 3 + color];
                }
                else{
                    grey_sum += 255;
                }
    ...
    if ((grey_sum - query_grey) <= TH2 && (grey_sum - query_grey) >= -1 * TH2){
        filtered[(start_row + subrow - 1 - x) * col * 3 + y * 3 + theta] = 1;
    }
    else{
        filtered[(start_row +subrow - 1 - x) * col * 3 + y * 3 + theta] = 0;
    }
}
...
int main(int argc, char **argv)
{
    ...
    cudaMalloc(&dimage, data_image_n * data_image_m*3*4);
    ...
    if (times != 0){
        ...
        {   
            if(start_row + part_row_size + query_row>row){
                size = row - start_row;
            }else{
                size = part_row_size+ query_row;
            }
            cudaMemcpy(dimage, &data_image[start_row*col*3], (size)*col*3*4, cudaMemcpyHostToDevice);
            cudaDeviceSynchronize();

            filter<<<blocks, threads>>>(dimage, drange, query_grey, part_row_size, col, query_row, query_col, TH2, dfiltered, start_row, row); // filtering
        }
    }

    part_row_size = row - part_row_size * times;
    int start_row = part_row_size * times;
    if (part_row_size != 0 && row-start_row>=query_row)
    {
        cudaMemcpy(dimage, &data_image[start_row*col*3], part_row_size*col*3*4, cudaMemcpyHostToDevice);
        int rooty = (col + 31) / 32;
        int rootx = (part_row_size + 31) / 32;
        dim3 blocks(rootx, rooty, 3);
        dim3 threads(32, 32, 1); // many extra threads will be there
    ...
    cudaMemcpyAsync(dimage, data_image, row*col*3*4, cudaMemcpyHostToDevice);
